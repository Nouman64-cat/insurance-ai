...
function DocCard({
  file, index, isActive, onSelect, onRemove,
}: {
  file: File; index: number; isActive: boolean;
  ...
  return (
    ...
    >
      ...
      <div className="px-2 pt-2.5">
        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${s.chip}`}>
          {s.glyph}
        </span>
      </div>
      ...
      <div className="px-2 pb-2.5 flex flex-col gap-0.5">
        <p className={`text-[10px] font-semibold leading-tight line-clamp-2 ${isActive ? "text-blue-700" : "text-slate-700"}`}>
          {file.name}
        </p>
...
function DocumentPreview({ file, objectURL }: { file: File; objectURL: string }) {
  const ext = getExt(file.name);
  if (ext === "pdf") {
    return <iframe src={objectURL} title="PDF Preview"
      className="w-full h-full rounded-lg border border-slate-200 bg-white" />;
  }
  if (IMAGE_PREVIEW.includes(ext)) {
    // eslint-disable-next-line @next/next/no-img-element
    return <img src={objectURL} alt="Document preview"
...
const OCR_STATUS: Record<OcrStatus, { label: string; color: string; dot: string }> = {
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="flex items-center gap-1">
            ...
            <span className="font-semibold text-slate-800">{usage.input.toLocaleString()}</span>
          </span>
          <span className="text-slate-300">·</span>
          <span className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
            <span className="text-slate-500">Out</span>
            <span className="font-semibold text-slate-800">{usage.output.toLocaleString()}</span>
          </span>
          <span className="text-slate-300">·</span>
          <span className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-violet-400" />
            <span className="text-slate-500">Total</span>
            <span className="font-semibold text-slate-800">{displayTotal.toLocaleString()}</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-400" />
            <span className="text-slate-500">In</span>
            <span className="font-semibold text-slate-700">{fmtCost(cost.input)}</span>
          </span>
          <span className="text-slate-300">·</span>
          <span className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
            <span className="text-slate-500">Out</span>
            <span className="font-semibold text-slate-700">{fmtCost(cost.output)}</span>
          </span>
          <span className="text-slate-300">·</span>
          <span className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400" />
            <span className="text-slate-500">Total</span>
            <span className="font-bold text-amber-700">{fmtCost(cost.total)}</span>
          </span>
        </div>
      </div>
    </div>
...
export default function OcrPage() {
  ...
  useEffect(() => {
    if (queueRef.current && files.length > 0) {
      queueRef.current.scrollTo({ left: queueRef.current.scrollWidth, behavior: "smooth" });
    }
  }, [files.length]);
  ...
  const addFiles = useCallback((incoming: FileList | File[]) => {
    ...
    if (invalid.length > 0) {
      setOcrError(`Unsupported: "${getExt(invalid[0].name)}". Accepted: ${SUPPORTED.join(", ").toUpperCase()}`);
      setOcrStatus("error");
      return;
    }
    setFiles((prev) => {
  ...
  const removeFile = (idx: number) => {
    setFiles((prev) => {
      ...
      if (next.length === 0) { setOcrStatus("idle"); setPreviewIdx(0); }
  ...
  const runOCR = async () => {
    ...
    try {
      ...
      for (const f of files) {
        ...
        form.append("file", f);
        const res = await ocrApi.post<{ filename: string; extracted_text: string; token_usage: TokenUsage }>("/extract", form);
  ...
  const runSummarizer = async () => {
    ...
    try {
      ...
      const res = await summarizerApi.post<{ summary: string; token_usage: TokenUsage }>("/summarize", body);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Extract and summarize insurance documents using Gemini 2.5 Flash.
          </p>
        </div>
            ...
            {files.length === 0 ? (
              ...
              >
                <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"
                    strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 text-slate-400">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">PDF · PNG · JPG · JPEG · TIFF · BMP</p>
                </div>
              </button>
            ) : (
            ...
            {activeFile && activeURL ? (
              <>
                ...
                <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-100 bg-slate-50">
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${extStyle(getExt(activeFile.name)).chip}`}>
                    {extStyle(getExt(activeFile.name)).glyph}
                  </span>
                  <p className="text-xs font-medium text-slate-600 truncate flex-1">{activeFile.name}</p>
                  <span className="text-[10px] text-slate-400 flex-shrink-0">{fmtSize(activeFile.size)}</span>
                </div>
              ...
              <div className="flex flex-col items-center justify-center h-full gap-2 text-slate-400">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"
                  strokeLinecap="round" strokeLinejoin="round" className="w-10 h-10 opacity-25">
                  <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" />
                  <polyline points="14 2 14 8 20 8" />
                  <line x1="16" y1="13" x2="8" y2="13" />
                  <line x1="16" y1="17" x2="8" y2="17" />
            ...
            {ocrStatus === "done" && (
              <div className="flex gap-2">
                ...
                <button
                  ...
                  `}
                >
                  {summaryStatus === "processing" ? <SpinnerIcon /> : (
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                      strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
                      <path d="M12 20h9" />
              ...
              <>
                {ocrStatus === "idle" && (
                  <div className="flex flex-col items-center justify-center h-full gap-3 text-slate-400">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"
                      strokeLinecap="round" strokeLinejoin="round" className="w-12 h-12 opacity-25">
                ...
                {ocrStatus === "ready" && (
                  <div className="flex flex-col items-center justify-center h-full gap-3 text-slate-400">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"
                      strokeLinecap="round" strokeLinejoin="round" className="w-12 h-12 opacity-35">
                      <polygon points="5 3 19 12 5 21 5 3" />
                    </svg>
                    <p className="text-sm font-medium text-slate-500">
                      {files.length} doc{files.length !== 1 ? "s" : ""} queued — click{" "}
                      <span className="font-bold text-blue-600">Run OCR Engine</span> to extract
                    </p>
                  </div>
                )}
                {ocrStatus === "processing" && (
                  <div className="flex flex-col items-center justify-center h-full gap-4">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-semibold text-slate-700">
                        Processing {files.length} doc{files.length !== 1 ? "s" : ""}…
                      </p>
                      <p className="text-xs text-slate-400 mt-1">Gemini 2.5 Flash is reading your documents</p>
                    </div>
                  </div>
                )}
                {ocrStatus === "error" && ocrError && (
                  <div className="flex flex-col items-center justify-center h-full gap-3">
                    ...
                    <div className="text-center max-w-sm">
                      ...
                      <p className="text-xs text-slate-500 mt-1 leading-relaxed">{ocrError}</p>
                    </div>
              ...
              <>
                {summaryStatus === "idle" && (
                  <div className="flex flex-col items-center justify-center h-full gap-3 text-slate-400">
                    ...
                    <p className="text-sm font-medium text-center max-w-xs">
                      {ocrStatus !== "done"
                        ? <>Run OCR first, then click <span className="font-bold text-violet-600">Summarize</span></>
                        : <>Click <span className="font-bold text-violet-600">Summarize</span> in the left panel</>
                      }
                    </p>
                  </div>
                )}
                {summaryStatus === "processing" && (
                  <div className="flex flex-col items-center justify-center h-full gap-4">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-semibold text-slate-700">Summarizing document…</p>
                      <p className="text-xs text-slate-400 mt-1">Gemini 2.5 Flash is analysing the extracted text</p>
                    </div>
                  </div>
                )}
                {summaryStatus === "error" && summaryError && (
                  <div className="flex flex-col items-center justify-center h-full gap-3">
                    ...
                    <div className="text-center max-w-sm">
                      <p className="text-sm font-semibold text-red-700">Summarization Failed</p>
                      <p className="text-xs text-slate-500 mt-1 leading-relaxed">{summaryError}</p>
                    </div>
                    <button onClick={runSummarizer}
