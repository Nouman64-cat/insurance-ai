      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-slate-200 overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="flex items-start gap-2 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2.5">
            ...
            <p className="text-xs text-blue-700">Coverage activates immediately upon binding. Payment is simulated (Pay-to-Bind gate — Phase 2).</p>
          </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {loading ? "Binding…" : "Bind & Issue Policy"}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 overflow-hidden">
        <div className="bg-gradient-to-r from-emerald-500 to-teal-500 px-6 py-5 text-center">
          ...
          <h2 className="text-white text-xl font-bold">Policy Issued!</h2>
          <p className="text-emerald-100 text-sm mt-1">{policyName}</p>
        </div>
        <div className="px-6 py-5 space-y-3">
          {[
            ...
            { label: "Total Annual Premium", value: fmtPKR(result.premium_breakdown.total_premium) },
...
function PolicyDetailsModal({ policy, onClose }: { policy: PolicyListItem, onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h2 className="text-lg font-bold text-slate-900 tracking-tight">Policy Details</h2>
            <p className="text-xs text-slate-500 font-mono mt-0.5">
              <span className="font-semibold text-slate-700">{policy.customer_name}</span> • {policy.policy_number}
            </p>
          </div>
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <span className="text-[10px] uppercase tracking-wider font-semibold text-slate-500">Customer</span>
              <p className="text-sm font-semibold text-slate-900">{policy.customer_name}</p>
            </div>
            <div className="space-y-1">
              <span className="text-[10px] uppercase tracking-wider font-semibold text-slate-500">Product</span>
              <p className="text-sm font-semibold text-slate-900">{policy.product_name}</p>
            </div>
            <div className="space-y-1">
              <span className="text-[10px] uppercase tracking-wider font-semibold text-slate-500">Sum Assured</span>
              <p className="text-sm font-semibold text-slate-900">{fmtCoverage(policy.coverage_amount)}</p>
            </div>
            <div className="space-y-1">
              <span className="text-[10px] uppercase tracking-wider font-semibold text-slate-500">Term</span>
              <p className="text-sm font-semibold text-slate-900">{policy.term_years} Years</p>
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <span className="text-[10px] uppercase tracking-wider font-semibold text-slate-500">Expiry Date</span>
              <p className="text-sm font-semibold text-slate-900">{policy.expiry_date ?? "—"}</p>
            </div>
          </div>
        </div>
...
export default function PolicyIssuancePage() {
  ...
  const active = useMemo(() =>
    policies.filter(p => p.status === "Active" || p.status === "ACTIVE"),
    [policies]);
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={k.title} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            <p className={`text-3xl font-bold mt-1 ${k.color === "amber" ? "text-amber-600" : k.color === "emerald" ? "text-emerald-600" : k.color === "red" ? "text-red-600" : "text-blue-600"}`}>
