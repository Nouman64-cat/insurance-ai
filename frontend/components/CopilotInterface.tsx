...
export function CopilotInterface() {
  ...
  return (
    <div className="flex flex-col h-full bg-white relative animate-in fade-in duration-500">
      ...
      {/* Background */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-blue-50/50 via-slate-50/30 to-white pointer-events-none" />
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 z-10 relative">
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.03] z-0">
          <img src="/rizvi.png" alt="" className="w-1/2 max-w-lg object-contain" />
        </div>
        <div className="max-w-4xl mx-auto space-y-8 pb-10 relative z-10">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} group animate-in slide-in-from-bottom-2 duration-300`}>
              ...
              <div className={`max-w-[80%] ${msg.role === "user" ? "items-end" : "items-start"}`}>
                <div className={`px-5 py-3.5 rounded-2xl text-[15px] leading-relaxed shadow-sm whitespace-pre-wrap flex flex-col gap-3 ${
                  msg.role === "user"
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {msg.quickActions.map((action, idx) => (
                        <button
                          ...
                          className={`px-3 py-1.5 text-[13px] font-bold rounded-lg border transition-all flex items-center gap-1.5 shadow-sm ${
          ...
          {isTyping && (
            <div className="flex justify-start items-center gap-2 ml-12">
              <div className="flex items-center gap-1.5 bg-white rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm border border-slate-100">
                {[0, 1, 2].map(i => (
