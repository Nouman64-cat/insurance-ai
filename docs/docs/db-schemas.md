...
| `code` | VARCHAR(50) | Unique, indexed; short SuperAdmin-assigned identifier (e.g. `ACME`) |
...
**Seeded roles:** `SuperAdmin`, `Admin`, `Underwriter`, `Agent`, `Viewer`. `SuperAdmin` users belong to a reserved `Platform` tenant (created by `create_superadmin.py`) — they create tenants and bootstrap each tenant's first `Admin` via `POST /tenants/{tenant_id}/setup`.
...
| `term_years` | INT | 1–40; further constrained per plan |
| `dependent_name` | VARCHAR(255) nullable | Only for `CHILD_EDUCATION_MARRIAGE` — the insured milestone belongs to a dependent, not the proposer |
| `dependent_dob` | DATE nullable | Only for `CHILD_EDUCATION_MARRIAGE`; validated against the plan's dependent age band |
| `created_at` | TIMESTAMP | UTC, auto-set |
...
| `composite_risk_score` | INT nullable | 0–100; weighted score from `decision_aggregation` |
| `ai_summary` | TEXT nullable | AI case summary generated by the Text Summarizer before evaluation |
| `created_at` | TIMESTAMP | UTC, auto-set |
